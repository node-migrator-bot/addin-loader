...
function findAddins(domain, searchPath, callback) {
    // look for addins in the html js addins directory for the domain
    fs.readdir(path.resolve(searchPath, 'js/addins/' + domain), function(err, files) {
        if (! err) {
...
function findCustomSkin(domain, searchPath, callback) {
    // look for addins in the html js addins directory for the domain
    fs.exists(path.resolve(searchPath, 'css/skins/' + domain + '.css'), function(exists) {
        if (exists && callback) {
...
function loadDomainConfig(mesh, domain, searchPath, appPath) {
    var configFile = path.resolve(searchPath, domain + '.json');
    
    fs.readFile(configFile, 'utf8', function(err, data) {
        if (! err) {
            mesh.log.info('loaded domain config: ' + configFile);
            ...
            // find the addins
            findAddins(domain, appPath, function(addins) {
                domainData[domain].addins = (domainData[domain].addins || []).concat(addins);
            });
            
            findCustomSkin(domain, appPath, function(skin) {
        ...
        if (! data) {
            for (domain in domainData) {
                ...
                if (matches) {
                    ...
                    log.info('referrer \'' + referer + '\' matched to domain: ' + domain);
...
exports.init = function(mesh, meshapp) {
    ...
    mesh.log.info('looking for loader configurations in: ' + searchPath);
    fs.readdir(searchPath, function(err, files) {
        ...
        for (var ii = 0; (! err) && ii < files.length; ii++) {
            loadDomainConfig(mesh, path.basename(files[ii], '.json'), searchPath, meshapp.basePath);
        } // for
    });    
};